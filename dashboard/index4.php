...
//$user_id = $_SESSION['id'];
    ...
    // }
      ...
      // include 'Auth/db/db_connection.php';
       ...
       $valid_formats = array( "png","PNG","JPG","JPEG","jpg");
$max_file_size = 1073741824; //100 kb
 ...
 foreach ($_FILES['files']['name'] as $f => $name) {
     ...
     if ($_FILES['files']['error'][$f] == 0) {
         if ($_FILES['files']['size'][$f] > $max_file_size) {
             $mess4 = "<span class='badge'>$name exceeded the number required four images!.</span>";
             $message[] = $mess4;  
     ...
     elseif( ! in_array(pathinfo($name, PATHINFO_EXTENSION), $valid_formats) ){
      $mess3 = "<span class='badge'>$name is not a valid format, use jpg,png or GIF </span>";
       $message[] = $mess3;
       echo $mess3;
     ...
     // echo "<script>$('#thankyouModal').modal('show')</script>";
       continue; // Skip invalid file formats
             ...
             if(move_uploaded_file($_FILES["files"]["tmp_name"][$f], $path.$name)) {
               ...
               $query ="insert into tbl_blog(title,description,article,img_name,img_path) values('$title','$desc','$article','$name','$path$name')";
               $ros = mysqli_query($con,$query);
               $_SESSION['insert_id']=mysqli_insert_id($con);
                $user_id = $_SESSION['insert_id'];
               if($ros!=true)
                                                                                                                                                                                                                                                                {
                $mess="<span class='text-danger'> Blog Not updated check your Database connection </span>";
                 $message[]= $mess;
...
echo $mess2;
        ...
        <form method="POST" enctype="multipart/form-data">
          <div class="col-md-6">
            <div class="form-group" style="margin-top:2%;">
             <label for="email" class="control-label" style="margin-top:4%;">Blog title</label>
              <input type="text" class="form-control" name="txt_title" placeholder="Blog Title" style="margin-bottom:2%; margin-top:4%;" required="">

                          <label for="email" class="control-label">Blog's Short Description</label>
               <textarea class="form-control" rows="5" id="comment" name="txt_desc" required=""></textarea>
          ...
          <span class="badge">To view full screen ctrl+Alt+F</span>
            <textarea id="tinymce_full" name="txt_article" required=""></textarea>
          </div>
      ...
      <input id="input-6" type="file" name="files[]" multiple="multiple" style="color:#000;" required="">
           
          <center><button class="btn btn-success btn-block" name="" style="margin-bottom: 4%; margin-top: 6%;">POST BLOG</button></center><br>
        </div>
        </div>
        </form>
        </div>  
  ...
  <div class="table-responsive">
    <table class="table table-hover table-striped" style="table-layout:fixed">
      ...
      while ($row = mysqli_fetch_array($run)) {
        ...
        $blog_img= $row['img_name'];
        $blog_post= $row['Date_posted'];
       ...
       <tr>
         ...
         <td><a href="Auth/delete.php?del=<?php echo $blog_id ?>"><button class="btn btn-danger">Delete Post</button></a></td>
       </tr>
       <?php } ?>
    </table>
  </div>
